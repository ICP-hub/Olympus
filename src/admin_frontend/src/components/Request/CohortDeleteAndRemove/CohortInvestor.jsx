...
function CohortInvestor({ allInvestorData, noData, cohortId }) {
  ...
  const handleSubmit = async () => {
    ...
    await actor
      .remove_vc_from_cohort(cohort_id, uid, passphrase_key)
      .then((result) => {
        if (
          result &&
          result?.Ok?.includes(
            `Venture capitalist successfully removed from the cohort with cohort id ${currentInvestorId}`
          )
        ) {
          toast.success("Investor successfully removed");
