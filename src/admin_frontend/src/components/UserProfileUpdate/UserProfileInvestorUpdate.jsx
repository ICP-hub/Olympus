...
const UserProfileInvestorUpdate = () => {
  ...
  const onSubmitHandler = async (data) => {
    if (actor) {
      ...
      try {
        await actor.update_vc_profile(principal,investorData).then((result) => {
          if (result && result.includes("Venture Capitalist profile updated successfully")) {
                    ...
                    {editMode ? (
                      ...
                      <div className="text-gray-600 flex flex-grow items-center mt-1 truncate text-xs">
                        ...
                        {updatedData?.multichain ? (
                          ...
                          <span>{updatedData?.multichain ? "Yes" : "No"}</span>
                        ) : (
                          <span className="flex items-center">
                            {noDataPresentSvg}
                            No Data
