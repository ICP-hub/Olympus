...
const MentorSideAssociation = () => {
  ...
  const fetchSelfRejectedRequestFromMentorToProject = async () => {
    try {
      const result =
        await actor.get_all_requests_which_got_self_declined_for_mentor();
      console.log(
        `result-in-get_all_requests_which_got_self_declined_for_mentor`,
        result
      );
      setData(result);
    } catch (error) {
      console.log(
        `error-in-get_all_requests_which_got_self_declined_for_mentor`,
        error
      );
      setData([]);
    }
          ...
          data.map((val, index) => {
            ...
            switch (activeTab) {
              case "pending":
                switch (selectedStatus) {
                  case "to-project":
                    img = val?.project_image[0]
                      ? uint8ArrayToBase64(val?.project_image[0])
                      : "";
                  ...
                  case "from-project":
                    img = val?.project_info?.project_logo
                      ? uint8ArrayToBase64(val?.project_info?.project_logo[0])
                      : "";
