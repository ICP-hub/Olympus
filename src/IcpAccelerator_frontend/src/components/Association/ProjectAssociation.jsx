...
const ProjectAssociation = () => {
  ...
  const [selectedStatus, setSelectedStatus] = useState(
    rolesFilterArray.length > 0 ? rolesFilterArray[0].id : ""
  );
  ...
  const handleApproveFunc = async (offer_id) => {
    ...
    try {
      const result = await actor.accept_offer_of_project(
        offer_id,
        response_message
      );
