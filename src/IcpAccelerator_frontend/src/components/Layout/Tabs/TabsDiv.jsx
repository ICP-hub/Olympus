...
export default function TabsDiv({ role, onClose }) {
  ...
  const getSpans = (roleName, status) => {
    return (
      <div className='border border-[#B8B8B8] flex items-baseline justify-between p-4 bg-gradient-to-l from-[#FFFFFF00] to-[#FFFFFF6B]'>
        ...
        {status === 'current' ? (
          ...
          <button
            ...
            className={`capitalize rounded-[44px] text-lg px-3 py-1 ${
              status === 'pending'
