...
import logoWithText from "../../../../assets/Logo/image 32.png";
import LogoutModal from "../../../models/LogoutModal";

const Header = ({ setModalOpen, gradient }) => {
  const connectedWalletPrincipal = useSelector( (currState) => currState.auth.principal);
  const internetIdentityPrincipal = useSelector((currState) => currState.internet.principal)
  const internetIdentityIsAuthenticated = useSelector((currState) => currState.internet.isAuthenticated)


  // console.log("principal in header", connectedWalletPrincipal);

  const manageHandler = () => {
    !connectedWalletPrincipal ? setModalOpen(true) : setModalOpen(false);
  };
  ...
  return (
    <header className={`text-gray-700 body-font ${gradient}`}>
     ...
     {internetIdentityPrincipal && internetIdentityIsAuthenticated ==true || connectedWalletPrincipal ? (
          <LogoutModal />
        ) : (
          <button
            type="button"
