...
//           ) : (
//             <>
//             {[...Array(latestAnnouncementData.length)].map((_, index) => (
//               <AnnouncementCardSkeleton key={index} />
//             ))}
//           </>
...
const AnnouncementCard = () => {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [currentAnnouncementData, setCurrentAnnouncementData] = useState(null);
  const [isLoading, setIsLoading] = useState(true); // Initially true to show skeleton if needed
  const [numSkeletons, setNumSkeletons] = useState(1);
  ...
  const handleAddOrUpdateAnnouncement = async (formData) => {
    setIsSubmitting(true);
    setIsLoading(true); // Show skeleton when updating or adding
    ...
    if (actor) {
      ...
      try {
        ...
        handleCloseModal();
        setTimeout(() => {
          fetchLatestAnnouncement(); // Re-fetch the latest announcements
        }, 500);
      } catch (error) {
        toast.error('Something went wrong');
      } finally {
  ...
  const handleDelete = async (announcementId) => {
    setIsSubmitting(true);
    setIsLoading(true); // Show skeleton when deleting
    ...
    try {
      ...
      fetchLatestAnnouncement(); // Re-fetch announcements after deletion
      handleCloseDeleteModal();
    } catch (error) {
      toast.error('Something went wrong');
    } finally {
  ...
  const fetchLatestAnnouncement = async () => {
    ...
    try {
      ...
      if (!result || result.length === 0) {
        setNoData(true); // Show NoCardData when there's no announcement
        setLatestAnnouncementData([]);
      } else {
        setLatestAnnouncementData(result);
        setNoData(false);
        setNumSkeletons(result.length); // Set the number of skeletons equal to the number of announcements
      }
    } catch (error) {
      setNoData(true);
      setLatestAnnouncementData([]);
      ...
      setTimeout(() => {
        setIsLoading(false); // End loading skeleton after 1 second
      }, 1000);
    }
  };
  ...
  return (
    <div className='bg-white w-full'>
      ...
      {noData && latestAnnouncementData.length === 0 ? (
        <div className='md:py-4 py-12'>
          <NoCardData />
          <button
            className='bg-[#155EEF] text-white px-2 md:px-4 py-2 rounded-md flex items-center justify-center mx-auto text-sm sm0:text-[16px]'
      ...
      ) : isLoading && latestAnnouncementData.length > 0 ? (
        <div className='md:py-4 py-12'>
          {Array(numSkeletons)
            .fill(0)
            .map((_, index) => (
