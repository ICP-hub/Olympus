...
import { investorRegistrationAdditionalInfo ,investorRegistrationPersonalDetails,investorRegistrationDetails} from "../../Utils/Data/InvestorFormData";
...
const InvestorRegistration = () => {
  ...
  const validationSchema = {
    personalDetails: yup.object().shape({
      email_address: yup.string().email('Invalid email format').required('Email address is required').test("is-non-empty", "Required", (value) => /\S/.test(value)),
      telegram_id: yup
      .string()
      .test("is-non-empty", "Required", (value) => /\S/.test(value))
      .required("Telegram ID is required"),
      portfolio_link: yup.string().url('Must be a valid URL').test("is-non-empty", "Required", (value) => /\S/.test(value)),
      location: yup.string().required('Location is required'),
      website_link: yup.string().url('Must be a valid URL').required('Website link is required'),
      technological_focus: yup.string().required('Technological focus is required'),
    ...
    investorDetails: yup.object().shape({
      ...
      typical_decision_making_timeline_for_investments: yup.string().required('Decision making timeline is required'),
      interest_in_board_positions: yup.boolean().required('Interest in board positions is required'),
      name_of_fund: yup.string().required('Name of fund is required'),
      size_of_managed_fund: yup.number().positive('Must be a positive number').required('Size of managed fund is required'),
      accredited_investor_status: yup.boolean().required('Accredited investor status is required'),
      preferred_investment_sectors: yup.string().required('Preferred investment sectors are required'),

    }),
    additionalInfo: yup.object().shape({
     average_investment_ticket: yup.number().positive('Must be a positive number').required('Average investment ticket is required'),
     investment_stage_preference: yup.string().required('Investment stage preference is required'),
  number_of_portfolio_companies: yup.number().positive('Must be a positive number').integer('Must be an integer').required('Number of portfolio companies is required'),
  revenue_range_preference: yup.string().required('Revenue range preference is required'),
  assets_for_investment: yup.number().positive('Must be a positive number').required('Assets for investment is required'),
  referrer: yup.string().test("is-non-empty", "Required", (value) => /\S/.test(value)),
    }),
  };
  ...
  const handleTabClick = async (tab) => {
    const targetStep = investorRegistration.findIndex((header) => header.id === tab);
