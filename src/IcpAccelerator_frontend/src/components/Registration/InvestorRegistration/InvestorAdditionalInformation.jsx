...
const InvestorAdditionalInformation = ({ onSubmit, register, errors, fields, goToPrevious,isSubmitting }) => {
  return (
    <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {fields.map((field) => (
            <div key={field.id} className="relative z-0 group mb-6">
              <label htmlFor={field.id} className="block mb-2 text-lg font-medium text-gray-500 hover:text-black truncate overflow-hidden text-start">
                {field.label}
              </label>
              <input
                type={field.type}
