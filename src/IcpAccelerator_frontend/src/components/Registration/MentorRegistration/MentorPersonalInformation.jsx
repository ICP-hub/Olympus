...
const MentorPersonalInformation = ({ onSubmit, register, errors, fields, goToNext }) => {
  return (
    <div>
      <form onSubmit={onSubmit} className="w-full px-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {fields.map((field) => (
            <div key={field.id} className="relative z-0 group my-3">
              <label htmlFor={field.id} className="block mb-2 text-lg font-medium text-gray-500 hover:text-black  hover:whitespace-normal truncate overflow-hidden hover:text-left">{field.label}</label>
              <input
                type={field.type}
                name={field.name}
                id={field.id}
