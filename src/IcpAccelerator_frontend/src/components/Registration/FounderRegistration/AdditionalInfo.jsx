...
import { additionalInfoFormFields } from "../../Utils/Data/founderFormField";

const schema = yup
  .object({
    reasonForJoin: yup.string().required("This field is required."),
    agreeToCommit: yup.string().required("This field is required."),
          ...
          ({ id, name, label, placeholder, type }) => (
            <div key={id} className="relative z-0 w-full mb-5 mt-6 group">
              <input
                ...
                type={type}
                id={id}
                className="block py-5 md:py-2.5 px-0 w-full text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-gray-200 peer"
                placeholder={placeholder}
              />
              <label
                htmlFor={id}
                className="peer-focus:font-medium absolute text-white duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-gray-300 peer-focus:dark:text-gray-200 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
              >
                {label}
              </label>
              {errors[name] && (
