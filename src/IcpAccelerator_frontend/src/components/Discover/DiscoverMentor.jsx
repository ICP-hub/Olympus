...
const DiscoverMentor = () => {
    ...
    await caller
      ...
      .then((result) => {
        if (isMounted) {
          if (result && result.data) {
            const mentorData = result.data ? Object.values(result.data) : [];
            ...
            setMentorData([]);
            setUserData([]); // Set to an empty array if no data
          }
          setIsLoading(false);
        }
      })
      .catch((error) => {
        if (isMounted) {
          ...
          setUserData([]);
          setIsLoading(false);
          console.log("error-in-get-all-mentor", error);
        }
      });
  };
  ...
  useEffect(() => {
    ...
    if (actor) {
      ...
      getAllMentor(IcpAccelerator_backend, isMounted);
    }
    ...
    <div>
      {isLoading ? (
        ...
        allMentorData.map((mentorArray, index) => {
          ...
          console.log("000000000000000000000", mentor);
