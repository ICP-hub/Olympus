...
const AssociationRequestCard = () => {
  ...
  const [selectedUser, setSelectedUser] = useState(null); // State to track selected user for detail view
  ...
  return (
    <div className="space-y-4">
        <div className='flex gap-3 items-center p-6 w-[650px]'><div className="flex items-center border-2 border-gray-400 rounded-lg overflow-hidden w-full h-[50px]">
    ...
    <FaFilter className="text-gray-400 text-2xl"/>
          ...
          <div className="flex-grow ml-[25px] w-[544px]">
            ...
            {/* <div className="mb-2">
              {user.tags.map((tag, index) => (
                <span
                  key={index}
                  className={`inline-block ${tagColors[tag] || "bg-gray-100 text-gray-800"} text-xs px-3 py-1 rounded-full mr-2 mb-2`}
                >
                  {tag}
                </span>
              ))}
      ...
      {/* {openDetail && selectedUser && (
        <UserDetailPage openDetail={openDetail} setOpenDetail={handleCloseDetail} user={selectedUser} />
      )} */}
    </div>
  );
};
