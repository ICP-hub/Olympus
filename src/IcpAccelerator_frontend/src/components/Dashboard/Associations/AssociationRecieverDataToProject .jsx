...
const AssociationRecieverDataToProject = ({
  ...
  const receiverData = {
    profile: {
      name:
        user?.reciever_data?.[0]?.[0]?.params?.project_name ??
        'Project Name not available',
      description:
        user?.reciever_data?.[0]?.[0]?.params?.project_description?.[0] ??
        'Project description not available',
      website:
        user?.reciever_data?.[0]?.[0]?.params?.project_website?.[0] ??
        'https://defaultwebsite.com',
      elevatorPitch:
        user?.reciever_data?.[0]?.[0]?.params?.project_elevator_pitch?.[0] ??
        'No project elevator pitch available',
      dappLink:
        user?.reciever_data?.[0]?.[0]?.params?.dapp_link?.[0] ??
        'No dapp link available',
      preferredIcpHub:
        user?.reciever_data?.[0]?.[0]?.params?.preferred_icp_hub?.[0] ??
        'Hub not available',
      longTermGoals:
        user?.reciever_data?.[0]?.[0]?.params?.long_term_goals?.[0] ??
        'Long term goals not available',
      revenue: user?.reciever_data?.[0]?.[0]?.params?.revenue?.[0] ?? 0n,
      weeklyActiveUsers:
        user?.reciever_data?.[0]?.[0]?.params?.weekly_active_users?.[0] ?? 0n,
      supportsMultichain:
        user?.reciever_data?.[0]?.[0]?.params?.supports_multichain?.[0] ??
        'Chain not available',
      technicalDocs:
        user?.reciever_data?.[0]?.[0]?.params?.technical_docs?.[0] ??
        'No technical docs available',
      tokenEconomics:
        user?.reciever_data?.[0]?.[0]?.params?.token_economics?.[0] ??
        'No token economics available',
      targetMarket:
        user?.reciever_data?.[0]?.[0]?.params?.target_market?.[0] ??
        'Target market not available',
      moneyRaised:
        user?.reciever_data?.[0]?.[0]?.params?.money_raised_till_now?.[0] ??
        false,
      isRegistered:
        user?.reciever_data?.[0]?.[0]?.params
          ?.is_your_project_registered?.[0] ?? false,
      isLiveOnIcp:
        user?.reciever_data?.[0]?.[0]?.params?.live_on_icp_mainnet?.[0] ??
        false,
      privateDocumentsUploaded:
        user?.reciever_data?.[0]?.[0]?.params?.upload_private_documents?.[0] ??
        false,
      typeOfRegistration:
        user?.reciever_data?.[0]?.[0]?.params?.type_of_registration?.[0] ??
        'Company',
      areaOfFocus:
        user?.reciever_data?.[0]?.[0]?.params?.project_area_of_focus ??
        'Area of Focus not available',
      projectCover: user?.reciever_data?.[0]?.[0]?.params?.project_cover?.[0]
        ? uint8ArrayToBase64(
            user?.reciever_data?.[0]?.[0]?.params?.project_cover?.[0]
          )
        : '../../../assets/Logo/CypherpunkLabLogo.png',
      projectLogo: user?.reciever_data?.[0]?.[0]?.params?.project_logo?.[0]
        ? uint8ArrayToBase64(
            user?.reciever_data?.[0]?.[0]?.params?.project_logo?.[0]
          )
        : '../../../assets/Logo/CypherpunkLabLogo.png',
    },
    user: {
  ...
  const senderData = {
    profile: {
      yearsOfMentoring:
        user?.sender_data?.[0]?.[0]?.profile?.years_of_mentoring ?? '0',
      icpHubOrSpoke:
        user?.sender_data?.[0]?.[0]?.profile?.icp_hub_or_spoke ?? false,
      hubOwner:
        user?.sender_data?.[0]?.[0]?.profile?.hub_owner?.[0] ??
        'Hub not available',
      website:
        user?.sender_data?.[0]?.[0]?.profile?.website?.[0] ??
        'https://defaultwebsite.com',
      existingIcpMentor:
        user?.sender_data?.[0]?.[0]?.profile?.existing_icp_mentor ?? false,
      categoryOfMentoringService:
        user?.sender_data?.[0]?.[0]?.profile?.category_of_mentoring_service ??
    ...
    <>
      <div className='md:py-6 w-full flex flex-wrap md:flex-nowrap rounded-lg shadow-sm'>
        ...
        <div className='flex-grow mt-4 md:mt-0 ml-0 md:ml-[25px]  w-full'>
          ...
          <p className='text-gray-600  overflow-hidden line-clamp-2 text-ellipsis min-h-20'>
              ...
              {selectedTypeData === 'to-project' ? (
                <button
                  ...
                  onClick={() =>
                    openModal({
                      ...
                      onConfirm: () => {
                        handleSelfReject(offerDetails?.offerId);
                        closeModal();
                      },
                    })
                  }
                ...
                <div className='flex flex-wrap sm:flex-nowrap'>
                  <button
                    ...
                    onClick={() =>
                      handleAcceptModalOpenHandler(offerDetails?.offerId)
                    }
                  >
                    Accept
                  </button>
                  <button
                    ...
                    onClick={() =>
                      handleDeclineModalOpenHandler(offerDetails?.offerId)
                    }
                  >
                    Reject
                  </button>
