...
const DocumentCard = ({ doc, type, projectId }) => {
  ...
  return (
    <div className={`relative flex items-center p-4 rounded-lg mb-4 shadow-md bg-white transition-all duration-300 ${isPrivate ? 'blur-sm hover:blur-none' : ''}`}>
      {/* Image Section with Background */}
      <div className="bg-gray-100 w-[180px] px-10 py-4 rounded-lg flex-shrink-0 text-center">
        <img
          src={Filetype}
      ...
      <div className="ml-4 flex-grow transition-all duration-300">
        <div className="flex justify-between">
          <p className={`text-lg font-semibold text-gray-900 ${isPrivate ? 'blur-sm hover:blur-none' : ''}`}>
            {title}
          </p>
          <button
            className={`rounded-lg px-2 text-sm ${
        ...
        <p className={`text-gray-600 mt-3 ${isPrivate ? 'blur-sm hover:blur-none' : ''}`}>
          {description}
        </p>
      </div>
