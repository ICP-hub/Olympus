...
const TeamCard = ({ member }) => {
  // Safely access profile picture using optional chaining
  const profile = member?.profile_picture?.[0]
    ? uint8ArrayToBase64(member?.profile_picture[0])
    : '';

  // Safely access social links using optional chaining
    ...
    <div className='flex flex-col sm0:flex-row gap-6 sm0:items-center p-4 bg-white shadow-lg border border-gray-200 rounded-xl mt-3 mb-4'>
      ...
      <div className='ml-4'>
        ...
        <p className='text-gray-500 text-sm line-clamp-1 break-all'>
          {member?.openchat_username ?? 'No Username'}
        </p>
        <p className='text-gray-600 text-base mt-2 line-clamp-2 break-all'>
          {member?.bio?.[0] ?? 'No bio available.'}
        </p>
...
const DiscoverTeam = ({ projectDetails }) => {
  console.log('discover team me gya m', projectDetails);

  // Safely access project team data using optional chaining
          ...
          {isLoading ? (
            <>
              {[...Array(members.length)].map((_, index) => (
                <DiscoverTeamSkeleton key={index} />
              ))}
            </>
          ) : (
