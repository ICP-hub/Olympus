...
const DiscoverMoneyRaising = ({ cardData, projectId }) => {
  ...
  const [approvedMoneyRaise, setApprovedMoneyRaise] = useState([]);
  ...
  const getAccessMoneyRaiseRequest = async (isMounted) => {
    ...
    await actor
      .access_money_details(projectId, convertedPrincipal)
      .then((result) => {
        if (isMounted) {
          console.log('access_money_details', result);
          if (result && result.Ok) {
            ...
            setNoData(true);
            toast.error('No data available from server');
          }
        }
      })
      .catch((error) => {
        if (isMounted) {
          console.log('error-access_money_details', error);
          setNoData(true); // Mark as no data on error
  ...
  useEffect(() => {
    ...
    if (actor) {
      getAccessMoneyRaiseRequest(actor, isMounted);
    }
    return () => {
      isMounted = false;
    };
  ...
  return (
    <div>
      <div className='max-w-7xl mx-auto bg-white mt-4'>
        {approvedMoneyRaise && approvedMoneyRaise ? (
          !noData ? (
            <DiscoverFundingCard
              data={approvedMoneyRaise}
              projectId={projectId}
              access={false}
            />
          ) : (
