...
const FilterModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 ${isOpen ? 'block' : 'hidden'}`}>
      <div
        className={`fixed inset-0 bg-black bg-opacity-50 transition-opacity duration-2000 ease-out ${isOpen ? 'opacity-100 delay-1000' : 'opacity-0'}`}
        onClick={onClose}
      ></div>
      <div
      ...
      >
        <div className='flex justify-between items-center mb-4'>
          ...
          <button className='text-[#364152] text-xl' onClick={onClose}>
            &times;
          </button>
        </div>
