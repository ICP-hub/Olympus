...
import { location, twitterSvg } from "../Utils/Data/SvgData";
...
function UserDetailPage() {
  ...
  const fetchUserProfile = async (caller) => {
    await caller
      .get_user_information_using_uid(id)
      .then((result) => {
        console.log("result-in-fetch-user-profile", result);
        setUserProfileData(result[0]);
      })
      .catch((error) => {
  ...
  useEffect(() => {
    if (actor) {
      fetchUserProfile(actor);
    } else {
      fetchUserProfile(IcpAccelerator_backend);
    }
  }, [actor]);

  console.log(userProfileData);
  //   let mentorName = userProfileData?.user_data?.full_name;
  //   let mentorprofile = uint8ArrayToBase64(
  //     userProfileData?.user_data?.profile_picture[0]
  //   );
  //   let mentorBio = userProfileData?.user_data?.bio[0];
  //   let mentorLocation = userProfileData?.user_data?.country;
  //   let areaOfExpertise = userProfileData?.user_data?.area_of_interest;
  //   let mentorWebsite = userProfileData?.website;
  //   let openchat_username = userProfileData?.user_data?.openchat_username;
  //   let twitter_id = userProfileData?.user_data?.twitter_id;
  //   let telegram_id = userProfileData?.user_data?.telegram_id;
  //   let linkedinLink = userProfileData?.linkedin_link;
  //   let email = userProfileData?.user_data?.email[0];
  //   let reason_to_join = userProfileData?.user_data?.reason_to_join;
  //   let preferred_icp_hub = userProfileData?.preferred_icp_hub?.[0];
