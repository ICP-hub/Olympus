...
import { JobBountieSkeleton } from "../../Dashboard/Skeleton/Jobsbountieskeleton";
...
const ProjectJobCard = ({ image, website, tags, country }) => {
  ...
  return (
    <>
      {isLoading ? (
        <div className="w-full grid gap-2 grid-cols-1 dxl0:grid-cols-2 md:px-4 md:gap-4 sm:gap-4">
          {Array(numSkeletons)
            .fill(0)
            .map((_, index) => (
              <JobBountieSkeleton key={index} />
            ))}
        </div>
      ) : noData ? (
