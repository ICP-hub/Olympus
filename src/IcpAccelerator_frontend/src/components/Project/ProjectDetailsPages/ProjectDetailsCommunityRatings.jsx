...
const ProjectDetailsCommunityRatings = ({
  data,
  isProjectLive,
  profile,
  type,
      ...
      {noData ? (
        ...
        <div className="md1:flex sm:flex flex-wrap ">
            ...
            <div className="w-full sm:w-full md:w-full p-4">
              <div className="shadow-md rounded-lg overflow-hidden drop-shadow-2xl gap-2 bg-blue-200 p-4 h-full">
                ...
                <p className="my-2 overflow-y-scroll text-[#737373]">
                  On basis of all Community ratings
                </p>
              </div>
            </div>
          </div>
          ...
          {cardData[0]?.map((val, index) => {
            ...
            let name = val[1]?.name ?? "";
            let image = val[1]?.profile_pic
              ? uint8ArrayToBase64(val[1]?.profile_pic)
              : "";
            let rating = val[1]?.rating;
            let message = val[1]?.message;
            let tag = val[1]?.tag;
            let date = val[1]?.timestamp
              ? formatFullDateFromBigInt(val[1]?.timestamp)
              : "";
            return (
              <div
                className="flex flex-wrap justify-center sm:justify-between w-full sm:w/1/2 md:w-1/3"
                key={index}
              >
                <div className="w-full p-4">
                  <div
                    className={`shadow-md rounded-lg overflow-hidden drop-shadow-2xl gap-2 p-4 ${bg_color}`}
                  >
                    <div className="flex flex-row justify-start sm:justify-start lg:justify-start flex-wrap gap-2 items-center">
                      ...
                      <div className="flex flex-col max-w-[120]">
                        ...
                        <p className="text-[#737373] flex items-center truncate">
                          {date}
                        </p>
                      </div>
                      <div className="flex flex-row justify-center items-center gap-2">
                    ...
                    <div className="flex flex-col ml-0 sm:ml-0 lg:ml-12">
                      ...
                      <p className="h-20 my-2 overflow-y-scroll text-[#737373]">
                        {message}
                      </p>
                    </div>
                  </div>
