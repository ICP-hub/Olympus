...
function SearchMentors() {
  ...
  const filteredInvestors = React.useMemo(() => {
    return allMentorData.filter(user => {
      ...
      const fullName = user[1]?.mentor_profile?.profile?.user_data?.full_name?.toLowerCase() || "";
      const areaoFInterest = user[1]?.mentor_profile?.profile?.user_data?.area_of_interest?.toLowerCase() || "";
            ...
            {currentMentors.map((mentor, index) => {
              ...
              return (
                <MentorCard
                  ...
                  bio={bio}
                  role={role}
                  category_of_mentoring_service={category_of_mentoring_service}
                  skills={skills}
                />
              );
            })}
