...
const InvestorProfileDetailsCard = ({
  data,
  isProjectLive,
  profile,
  type,
    ...
    <>
      {addButton && isProjectLive && (
        <div className="w-full flex justify-end  mb-4">
          <button
            onClick={() => navigate(`/view-investor/${data?.uid}`)}
      ...
      <div className="md:flex">
        {data?.params?.vc_assigned && data?.params?.vc_assigned.length > 0 ? (
          data?.params?.vc_assigned[0]?.map((data, index) => {
            ...
            let image = data?.user_data?.profile_picture[0]
              ? uint8ArrayToBase64(data?.user_data?.profile_picture[0])
              : "";
                  ...
                  <div className=" relative shadow-md rounded-lg overflow-hidden  drop-shadow-xl gap-2 bg-white h-32 ">
                    ...
                    <div className="absolute flex  flex-row gap 2 p-4 mt[-20px] gap-4">
                      ...
                      <div className="flex-col flex mt-4">
                        <p className="text-black font-bold tex-lg md:text-2xl">
                          {name}
                        </p>
                        <p>Investor</p>
                      </div>
