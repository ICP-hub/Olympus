...
use crate::{
    find_project_by_id, get_mentor_by_principal, mentor, mutate_state, APPLICATION_FORM,
    MENTOR_REGISTRY,
};
        ...
        if let Some(offers) = &mut state.mentor_alerts.get(&StoredPrincipal(mentor_id)) {
            if let Some(offer) = offers.0.iter_mut().find(|o| o.offer_id == offer_id) {
                    ...
                    APPLICATION_FORM.with(|projects| {
                        ...
                        if let Some(project) = projects.get_mut(&offer.sender_principal) {
                            if let Some(project) = project
                                .iter_mut()
                                .find(|p| p.uid == offer.project_info.project_id)
                            {
                                if project.params.mentors_assigned.is_none() {
                                    project.params.mentors_assigned = Some(Vec::new());
                                }
                                ...
                                mutate_state(|storage| {
                                    let associate_project =
                                        &mut storage.projects_associated_with_mentor;
                ...
                mutate_state(|sent_state| {
                    if let Some(sent_status_vector) = &mut sent_state
                        .my_sent_notifications
