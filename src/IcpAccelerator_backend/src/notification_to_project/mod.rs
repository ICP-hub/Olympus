                                    ...
                                    if let Some(project) = projects.get_mut(&offer.sender_principal)
                                    {
                                            ...
                                            .find(|p| p.uid == project_offer.project_id)
                                        {
                                            ...
                                            if !mentors_assigned.contains(&mentor_profile.profile) {
                                                mentors_assigned
                                                    .push(mentor_profile.profile.clone());
                                            }
                                            ...
                                            mutate_state(|storage| {
                                                ...
                                                let mut projects = associate_project
                                                    .get(&crate::StoredPrincipal(
                                                        offer.sender_principal,
                                                    ))
...
pub fn self_decline_request_for_mentor(offer_id: String) -> String {
    ...
    mutate_state(|sent_ones| {
        let mut my_offers = &mut sent_ones.my_sent_notifications_project;
    ...
    if response == "Request got self declined." {
        mutate_state(|mentors| {
            let mut mentor_offers = &mut mentors.project_alerts;
