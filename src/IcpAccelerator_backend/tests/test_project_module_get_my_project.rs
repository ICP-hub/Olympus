...
#[test]
fn test_get_my_project_successful() {
    ...
    // Call the get_my_project function
    ...
    let (retrieved_project_info, retrieved_user_info): (ProjectInfoInternal, UserInfoInternal) = decode_args(&response).expect("Decoding failed");

    assert_eq!(retrieved_project_info.params.project_name, project_info.params.project_name, "Project name should match");
    assert_eq!(retrieved_user_info.params.full_name, user_info.params.full_name, "User name should match");
    assert_eq!(retrieved_user_info.params.area_of_interest, user_info.params.area_of_interest, "Profile picture should match");
}

#[test]
#[should_panic(expected = "Couldn't get a project")]
fn test_get_my_project_no_project() {
    let (pic, backend_canister) = setup();
    ...
    // Call the get_my_project function, expecting it to panic due to no project
    pic.query_call(
        backend_canister,
        test_principal,
        "get_my_project",
...
#[test]
fn test_get_my_project_no_user_info() {
    let (pic, backend_canister) = setup();
   ...
   // Simulate the project registration by directly manipulating the canister state
    ...
    // Call the get_my_project function, expecting it to panic due to no user information
    pic.query_call(
        backend_canister,
        test_principal,
        "get_my_project",
