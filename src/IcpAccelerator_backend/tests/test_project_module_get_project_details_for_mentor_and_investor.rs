...
#[test]
fn test_get_project_details_for_mentor_and_investor() {
    ...
    // Call the get_project_details_for_mentor_and_investor function
    let Ok(WasmResult::Reply(response)) = pic.query_call(
        backend_canister,
        test_principal,
        "get_project_details_for_mentor_and_investor",
        encode_one(project_id).unwrap(),
    ) else {
        panic!("Expected reply");
    };
...
fn test_get_project_details_for_mentor_and_investor_not_found() {
    let (pic, backend_canister) = setup();
    ...
    // Call the get_project_details_for_mentor_and_investor function with an invalid project ID
